...
static int can_dump_inet_sk(const struct inet_sk_desc *sk)
{
	...
	case TCP_SYN_SENT:
		...
		break;
       #ifdef SCTP_LISTEN_DEF
	case SCTP_LISTEN:
		//PRIYA: FIX the sctp skip in flight case
		#ifdef sctp_skip_in_flight
		if (sk->rqlen != 0) {
			if (opts.sctp_skip_in_flight) {
				pr_info("Skipping in-flight connection (l) for %x\n",
						sk->sd.ino);
				break;
			}
...
static struct inet_sk_desc *gen_uncon_sk(int lfd, const struct fd_parms *p, int proto, int family, int type)
{
	...
	if (type != SOCK_RAW && proto == IPPROTO_SCTP) {
		struct sctp_status info = {0};

		aux = sizeof(info);
		ret = getsockopt(lfd, SOL_SCTP, SCTP_STATUS, &info, &aux);
		if (ret) {
			pr_perror("Failed to obtain SCTP_STATUS");
			goto err;
		}
		pr_err("SCTP Socket state %d\n", info.sstat_state);

		if (info.sstat_state != SCTP_CLOSED) {
			pr_err("Socket state %d obtained but expected %d\n",
			       info.sstat_state, SCTP_CLOSED);
			goto err;
		}
...
static int do_dump_one_inet_fd(int lfd, u32 id, const struct fd_parms *p, int family)
{
	...
	pr_err("Socket state %d family %d proto %d\n",sk->state, family, proto);
	...
	case IPPROTO_TCP:
		...
		break;
	case IPPROTO_SCTP:
		err = (type != SOCK_RAW) ? dump_one_sctp(lfd, sk) : 0;
		break;
	case IPPROTO_UDP:
	case IPPROTO_UDPLITE:
		sk_encode_shutdown(&ie, sk->shutdown);
...
static inline int sctp_connection(InetSkEntry *ie)
{
	return (ie->proto == IPPROTO_SCTP && ie->dst_port);
}
...
static int collect_one_inetsk(void *o, ProtobufCMessage *base, struct cr_img *i)
{
	...
	if (sctp_connection(ii->ie))
		sctp_locked_conn_add(ii);
...
static int post_open_inet_sk(struct file_desc *d, int sk)
{
	...
	if (sctp_connection(ii->ie)) {
			pr_debug("Schedule %d sctp socket for repair off\n", sk);
			BUG_ON(ii->sk_fd != -1);
			ii->sk_fd = sk;
			return 0;
	}
...
static int open_inet_sk(struct file_desc *d, int *new_fd)
{
	...
	if (tcp_connection(ie)) {
		pr_err("calling restore tcp socket\n");
		if (!opts.tcp_established_ok && !opts.tcp_close) {
	...
	if (sctp_connection(ie)) {

			pr_err("calling restore sctp socket\n");
			/* Priya: Not clear about this opts
			if (!opts.sctp_established_ok && !opts.sctp_close) {
				pr_err("Connected SCTP socket in image\n");
				goto err;
			} */

			mutex_lock(&ii->port->reuseaddr_lock);
			if (restore_one_sctp(sk, ii)) {
				mutex_unlock(&ii->port->reuseaddr_lock);
				goto err;
			}
			mutex_unlock(&ii->port->reuseaddr_lock);
	...
	if (ie->state == TCP_LISTEN) {
		if ((ie->proto != IPPROTO_TCP) && (ie->proto != IPPROTO_SCTP)) {
			pr_err("Wrong socket in listen state %d\n", ie->proto);
			goto err;
		}

		if(ie->proto == IPPROTO_SCTP) {
			...
			init.sinit_num_ostreams = 8;
			init.sinit_max_instreams = 8;
			init.sinit_max_attempts = 0;
			init.sinit_max_init_timeo = 0;

			if (setsockopt (sk, IPPROTO_SCTP, SCTP_INITMSG, &init, sizeof (struct sctp_initmsg)) < 0) {
				pr_err("SCTP stream set failure\n");
			}
			...
			if (setsockopt (sk, IPPROTO_SCTP, SCTP_EVENTS, &event, sizeof (struct sctp_event_subscribe)) < 0) {
				pr_err("SCTP event set failure\n");
			  } */
		}
	...
	/*if (ie->state == SCTP_LISTENING) {
		pr_err("SCTP listening socket restored\n");
		if (ie->proto != IPPROTO_SCTP) {
			pr_err("Wrong socket in listen state %d\n", ie->proto);
			goto err;
		}
